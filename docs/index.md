---
layout: home

hero:
  name: "온톨로지 학습 교안"
  text: "지식그래프를 손으로 만들며 배우기"
  tagline: 트리플 하나에서 시작해, 서초 AI 페스타에 온톨로지를 적용하기까지.
  actions:
    - theme: brand
      text: M0부터 시작하기
      link: /modules/m0-triple
    - theme: alt
      text: 부록 & 진도표
      link: /appendix
    - theme: alt
      text: 🔗 실습실 레포 (GitHub)
      link: https://github.com/tykimos/onto-osint-yuseong-event

features:
  - icon: "🧩"
    title: "M0. 멘탈모델 — 트리플"
    details: "모든 지식을 주어-서술어-목적어로 쪼개기. 온톨로지의 심장."
    link: /modules/m0-triple
  - icon: "🗂️"
    title: "M1. 클래스 & 인스턴스"
    details: "세상을 '종류'로 나누기. schema.json ↔ instances.json"
    link: /modules/m1-class-instance
  - icon: "🔗"
    title: "M2. 속성 & 관계"
    details: "개체를 잇기. operates / hostsAt / partOfSeries"
    link: /modules/m2-property-relation
  - icon: "🧠"
    title: "M3. 추론"
    details: "적지 않은 사실을 컴퓨터가 만들어낸다. 신뢰도 점수까지."
    link: /modules/m3-reasoning
  - icon: "🌐"
    title: "M4. 지식그래프 & 시각화"
    details: "KG 스냅샷, 스키마 자동 진화, 그리고 온토에어 연결점."
    link: /modules/m4-knowledge-graph
  - icon: "🚀"
    title: "M5. 실전 — 서초 AI 페스타 온톨로지"
    details: "onto-osint fork → 서초 AI 페스타 일일 큐레이션 파이프라인 완성."
    link: /modules/m5-build
---

## 이 교안 사용법

::: tip 💡 쉽게 썼어요
모든 개념을 **초등학생도 알 수 있게 쉬운 비유**로 풀었습니다. 단, **기술 용어(트리플·클래스·인스턴스·추론·지식그래프 등)는 그대로** 두고 바로 옆에 쉬운 풀이를 붙였어요. 용어와 친해지는 게 목표니까요. 모르는 단어가 나오면 가까운 **💡 박스**를 보세요.
:::

이 교안을 끝내면 **서초 AI 페스타 일일 큐레이션 파이프라인**이 완성돼 있을 거예요. 책처럼 추상적으로가 아니라, **손으로 익혀서 실제로 쓰는** 것을 목표로 합니다.

- **[개념]** → 핵심만 짧게 이해하는 파트
- **[레포에서 볼 곳]** → [`onto-osint` 실습실 레포](https://github.com/tykimos/onto-osint-yuseong-event)에서 실물로 확인
- **[실습]** → 내 도메인(서초 AI 페스타 / 부동산)으로 손으로 옮겨보기
- **[완료 체크]** → 다음으로 넘어가도 되는지 확인

> **실습실(lab):** [`tykimos/onto-osint-yuseong-event`](https://github.com/tykimos/onto-osint-yuseong-event) — 크롤링+온톨로지로 지역 이벤트 일일 큐레이션 리포트를 만드는 레포
> **이번 프로젝트 범위:** 서초 AI 페스타(10/17)에 온톨로지 적용 — 일일 큐레이션 파이프라인 완성까지 (부동산·온토에어는 다음 범위)
> **권장 페이스:** 1주 1모듈. 9월 전 M5 완성.

**읽기만 하지 말고, 매 모듈 실습을 꼭 완성하세요.** 그게 수업입니다.
